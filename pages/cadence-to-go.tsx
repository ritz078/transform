...
async function cadenceToGo(value: string): Promise<string> {
  const parser = await CadenceParser.create(
    "https://cdn.jsdelivr.net/npm/@lemonneko/flow__cadence-parser@0.24.1/dist/cadence-parser.wasm"
  );
    ...
    if (dec.CompositeKind === "CompositeKindStructure") {
      ...
      for (const member of dec.Members.Declarations) {
        if (member.Type === "FieldDeclaration") {
          decRet += `${firstLetterUppercase(member.Identifier.Identifier)} ${
            typeMap[member.TypeAnnotation.AnnotatedType.Identifier.Identifier]
          } \`godence:"${member.Identifier.Identifier}"\`\n`;
        }
      }
      decRet += "\n}";
    }
